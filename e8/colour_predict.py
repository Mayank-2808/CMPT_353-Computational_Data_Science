...
OUTPUT_TEMPLATE = (
    'Bayes Classifier:        {bayes_rgb:.3f} {bayes_convert:.3f}\n'
    'kNN Classifier:          {knn_rgb:.3f} {knn_convert:.3f}\n'
    'Random Forest Classifier: {rf_rgb:.3f} {rf_convert:.3f}\n'
)

# Representative RGB colors for each label, for nice display
COLOR_RGB = {
    'red': (255, 0, 0),
    'orange': (255, 113, 0),
    'yellow': (255, 255, 0),
...
name_to_rgb = np.vectorize(COLOR_RGB.get, otypes=[np.uint8, np.uint8, np.uint8])
...
def main():
    ...
    rf_convert_model = make_pipeline(FunctionTransformer(convert_RGBtoLAB), RandomForestClassifier(n_estimators=200))

    # Train each model and output predictions
