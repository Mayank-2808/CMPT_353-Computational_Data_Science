...
def pivot_months_pandas(data):
    ...
    grouped_data = data.groupby(['name', 'month']).agg({'precipitation': 'sum'}).reset_index()

    monthly = grouped_data.pivot(index='name', columns='month', values='precipitation').fillna(0)

    counts = data.groupby(['name', 'month']).size().reset_index(name='count').pivot(index='name', columns='month', values='count').fillna(0)
